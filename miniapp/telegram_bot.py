...
async def _send_setup_complete_notification(context: ContextTypes.DEFAULT_TYPE, target_id: str, target_lang: str):
    """Send setup completion notification and dynamically generated menu to user.
    
    Sends two messages to the user:
    1. Setup confirmation message ("setup_configured" text)
    2. Welcome message with dynamically generated interactive menu
    
    The menu is generated by _user_menu_keyboard(target_id) which returns an
    InlineKeyboardMarkup. The menu automatically includes the "Add more cities" button
    for users with active subscriptions (trial or paid). Users without active
    subscriptions will see only the "Try 14 days FREE" button.
    ...
    Dependencies:
        - _user_menu_keyboard(uid): Returns InlineKeyboardMarkup with menu buttons
        - get_text(key, lang): Returns localized text for the given key and language
    ...
    try:
        ...
        # Show updated menu with all buttons including "Add more cities" for active subscriptions
...
async def enter_links_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "trial":
        ...
        await _send_setup_complete_notification(context, target_id, target_lang)
